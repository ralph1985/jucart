...
El Hito 36 crea un perfil privado en `public.user_profiles` para cada cuenta de Supabase Auth mediante un trigger sobre `auth.users`. El perfil se actualiza si cambia el email y sus políticas RLS solo permiten leerlo o modificarlo a la propia cuenta. La asociación de la lista actual con Rafa queda para el Hito 37, cuando exista la tabla de listas y miembros; hasta entonces la lista configurada por `list_id` sigue funcionando con el acceso anónimo compatible.

El Hito 37 introduce `shopping_lists` y `shopping_list_members`. La lista actual conserva su UUID y sus 95 productos, queda asociada a Rafa como `owner` y mantiene su código de unión en la nueva tabla. Las operaciones de crear lista, unirse mediante código, regenerar el código y abandonar una lista se ofrecen mediante RPC; la pertenencia se consulta con RLS. La gestión inicial se muestra en el menú `Dev` junto al acceso Auth para validar el cambio sin añadir rutas ni alterar la pantalla principal. Una cuenta autenticada sin listas ve allí la invitación a crear una o introducir un código. La lista activa se guarda localmente y se recarga la aplicación con el `listId` seleccionado. Begoña se añadirá cuando cree su cuenta Auth y use el código de la lista, porque todavía no existe una identidad Auth suya que pueda asociarse sin inventar datos.
