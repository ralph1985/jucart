...
La aplicación usa cuentas individuales gestionadas con Supabase Auth y exige iniciar sesión con email y contraseña antes de mostrar datos cuando Supabase está configurado. Se evita el email transaccional de Supabase porque sus límites no encajan con el uso de pruebas; las dos cuentas iniciales se provisionan directamente y las contraseñas las almacena Supabase mediante hash. El selector manual se conserva solo para atribuir correctamente el histórico mientras se completa la transición, y el acceso anónimo queda revocado en la base de datos.

Las listas son listas compartidas por supermercado. No se crea una lista por usuario ni existe el concepto de casa: las listas antiguas con productos se conservan como siete listas independientes, Rafa es propietario de todas y Begoña miembro de todas. La pertenencia se asocia a una cuenta y tiene uno de dos roles: `owner` o `member`. Ambos roles pueden leer y modificar el contenido; solo el propietario puede regenerar el código de unión. Durante esta transición no se ofrecen nuevas listas en la interfaz.

El Hito 36 crea un perfil privado en `public.user_profiles` para cada cuenta de Supabase Auth mediante un trigger sobre `auth.users`. El perfil se actualiza si cambia el email y sus políticas RLS solo permiten leerlo o modificarlo a la propia cuenta. También crea o actualiza las cuentas iniciales de Rafa (`rafaelgarcia1985@hotmail.com`) y Begoña (`bego15val@gmail.com`) y deja el login por contraseña como puerta de entrada obligatoria.

El Hito 37 introduce `shopping_lists` y `shopping_list_members` y migra las listas antiguas que tenían productos: Mercadona, Alcampo, Día, Carrefour, Farmacia, Amazon y Leroy Merlín. Se conservan productos, secciones, tickets, historial, precios y datos técnicos asociados; Rafa queda como `owner` y Begoña como `member` en las siete. La aplicación carga simultáneamente todas las listas a las que pertenece el usuario, como propietario o miembro, y mantiene el `list_id` únicamente como clave interna de cada fila. El menú `Listas` identifica las listas compartidas durante la transición, pero no crea más listas ni cambia el contexto de la pantalla principal.

Durante la transición, cada lista muestra el email de su propietario. Se obtiene mediante una RPC limitada a las listas del usuario autenticado, sin hacer públicos los perfiles completos ni relajar el RLS de `user_profiles`.
