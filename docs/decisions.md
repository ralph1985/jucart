...
El Hito 37 introduce `shopping_lists` y `shopping_list_members` y migra las listas antiguas que tenían productos: Mercadona, Alcampo, Día, Carrefour, Farmacia, Amazon y Leroy Merlín. Se conservan productos, secciones, tickets, historial, precios y datos técnicos asociados; Rafa queda como `owner` y Begoña como `member` en las siete. La lista activa se guarda localmente y se recarga la aplicación con el `listId` seleccionado. El menú `Dev` permite consultar el propietario y cambiar entre las listas compartidas durante la transición, pero no crea más listas.
