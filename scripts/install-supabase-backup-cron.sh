...
BACKUP_ENV_FILE="${JUCART_SUPABASE_BACKUP_ENV_FILE:-$HOME/.config/jucart/supabase-backup.env}"
SCHEDULE="${JUCART_SUPABASE_BACKUP_CRON_SCHEDULE:-0 */6 * * *}"
CRON_COMMAND="JUCART_SUPABASE_BACKUP_ENV_FILE=$BACKUP_ENV_FILE $REPO_ROOT/scripts/backup-supabase.sh >> $LOG_DIR/supabase-backup.cron.log 2>&1"
